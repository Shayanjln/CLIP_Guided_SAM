...
from .common import LayerNorm2d, MLPBlock, ConvBlock, Conv_Scale_Block, Conv_All_Scale_Block, Conv_Adapter_All_Scale_Block, Adapter_Block
...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        for i in range(depth):
            ...
            convblock = Adapter_Block(embed_dim,16)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for blk,convblk in zip(self.blocks,self.convblocks):
            xt = x
            xc = convblk(xt + self.scale1*xc)
            #x = blk(xt)
        ########
            x = blk(xt + self.scale2*xc)
