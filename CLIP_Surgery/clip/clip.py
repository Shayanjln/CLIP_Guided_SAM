...
def encode_text_with_prompt_ensemble(model, texts, device, prompt_templates=None):
    ...
    for t in texts:
        ...
        class_embeddings = model(prompted_t,'text')
        class_embeddings = class_embeddings/class_embeddings.norm(dim=-1, keepdim=True)
        class_embedding = class_embeddings.mean(dim=0)
        class_embedding = class_embeddings/class_embedding.norm()
        text_features.append(class_embedding)
    tx= torch.stack(text_features, dim=1)
...
def clip_feature_surgery(image_features, text_features, redundant_feats=None, t=2):
    ...
    if redundant_feats != None:
        ...
        #w = w.reshape(w.shape[0], 1, w.shape[2], 1)  # Shape: [B, 1, N_T, 1]
        w = w.reshape(b, 1, n_t, 1)
        feats *= w  
