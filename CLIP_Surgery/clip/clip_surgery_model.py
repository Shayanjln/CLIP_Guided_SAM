    ...
    def forward(self, x):
        if isinstance(self.attn, Attention):
            if isinstance(x, list):
                ...
                x_ori = x_ori + x_ori_res  # ✅ No in-place modification
                x_ori = x_ori + self.mlp(self.ln_2(x_ori))  # ✅ No in-place modification
                x = x + x_res  # ✅ No in-place modification
                ...
                if isinstance(x_res, list):
                    x_res, x_ori_res = x_res
                    x_ori = x.clone() + x_ori_res  # ✅ Clone ensures a new tensor
                    x_ori = x_ori + self.mlp(self.ln_2(x_ori))  # ✅ No in-place modification
                    x = x + x_res  # ✅ No in-place modification
                    return [x, x_ori]
    
        else:
            x = x + self.attention(self.ln_1(x))  # ✅ No in-place modification
            x = x + self.mlp(self.ln_2(x))  # ✅ No in-place modification
        return x
...
class CLIPSurgery(nn.Module):
    ...
    def forward_orig(self, image, text):
    ...
    def forward(self,x,mode="image"):
        if mode=="image":
            ...
            raise ValueError(f"Invalid mode '{mode}'. Expected 'image' or 'text'.")
